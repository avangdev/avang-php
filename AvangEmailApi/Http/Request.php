<?php
/**
 * This file contains the AvangEmailApi_Http_Request class used in the AvangEmailApi PHP-SDK.
 *
...
/**
 * AvangEmailApi_Http_Request is the request class used to send the requests to the API endpoints.
 *
 * @author Serban George Cristian <cristian.serban@mailwizz.com>
 ...
 * @since 1.0
 */
class AvangEmailApi_Http_Request extends AvangEmailApi_Base
{
    /**
     * @var AvangEmailApi_Http_Client the http client injected.
     */
    public $client;

    /**
     * @var AvangEmailApi_Params the request params.
     */
    public $params;
     ...
     * Constructor.
     *
     * @param AvangEmailApi_Http_Client $client
     */
    public function __construct(AvangEmailApi_Http_Client $client)
    {
        $this->client = $client;
    }
    ...
    public function send()
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT , 'AvangEmailApi Client version '. AvangEmailApi_Http_Client::CLIENT_VERSION);
        ...
        if ($client->isPostMethod || $client->isPutMethod || $client->isDeleteMethod) {
            ...
            $params = new AvangEmailApi_Params($client->paramsPost);
        ...
        $params = $this->params = new AvangEmailApi_Params($curlInfo);
        ...
        if ($curlCode === 0 && $client->getResponseHeaders) {
            ...
            $params->add('headers', new AvangEmailApi_Params($headers));
        }

        $decodedBody = array();
        if ($curlCode === 0 && !empty($body)) {
            $decodedBody = AvangEmailApi_Json::decode($body, true);
        ...
        $params->add('body', new AvangEmailApi_Params($decodedBody));

        $response = new AvangEmailApi_Http_Response($this);
        $body = $response->body;
    ...
    protected function sign($requestUrl)
    {
        ...
        $params = new AvangEmailApi_Params($specialHeaderParams);
